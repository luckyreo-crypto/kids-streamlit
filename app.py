...
st.markdown("""
    <style>
    /* ✅ 13, 14. 부드러운 스크롤 & 더블 탭 줌 방지 */
    html { scroll-behavior: smooth; }
    button, input, select, textarea, div[data-testid="stToggle"] { touch-action: manipulation !important; }

    /* ✅ 3. 앱 전역 폰트 크기 상향 */
    html, body, [class*="css"] { font-size: 16px !important; }

    /* ✅ 1, 2, 15. 상단 메뉴(Tabs) 모바일 가로 스크롤 및 크기 대폭 확대 */
    ...
    div[data-baseweb="tab-list"] { 
        ...
        -webkit-overflow-scrolling: touch !important; padding-bottom: 5px !important; scrollbar-width: none !important;
    }
    div[data-baseweb="tab-list"]::-webkit-scrollbar { display: none !important; }
    div[data-baseweb="tab"] { 
        flex: 0 0 auto !important; justify-content: center; padding: 12px 20px !important; margin: 0 5px 0 0 !important; 
        background-color: #f8f9fa; border-radius: 8px; border: 1px solid #eee; min-height: 48px !important;
    }
    ...
    /* ✅ 6, 7. 모든 버튼 및 입력창 터치 규격 최적화 (iOS 자동확대 방지 16px 적용) */
    div[data-testid="stButton"] button { min-height: 48px !important; font-size: 1.05rem !important; font-weight: 600 !important; border-radius: 8px !important; }
    input, select, textarea { min-height: 48px !important; font-size: 16px !important; border-radius: 8px !important; padding: 10px !important; }
    
    /* ✅ 18. 체크박스/라디오버튼 라벨 확대 */
    div[data-testid="stRadio"] label p, div[data-testid="stCheckbox"] label p { font-size: 1.05rem !important; font-weight: 600 !important; }
    
    /* ✅ 9. 아코디언(Expander) 헤더 확대 */
    div[data-testid="stExpander"] summary { min-height: 54px !important; font-size: 1.1rem !important; font-weight: 700 !important; }
    
    /* ✅ 12. 파일 업로더 드래그 영역 확대 */
    div[data-testid="stFileUploader"] section { min-height: 120px !important; border: 2px dashed #0366d6 !important; background-color: #f8fbff !important; }

    /* ✅ 19. 대시보드 통계 수치 1.5배 확대 */
    div[data-testid="stMetricValue"] { font-size: 2.2rem !important; font-weight: 900 !important; color: #0366d6 !important; }
    ...
    .fab-button { position: fixed; bottom: 30px; left: 30px; background-color: rgba(3, 102, 214, 0.9); color: white !important; padding: 14px 24px; border-radius: 30px; text-decoration: none; font-weight: bold; font-size: 1rem; box-shadow: 0 4px 10px rgba(0,0,0,0.3); z-index: 999999; }
    
    /* 🔴 카드 UI 패딩 최적화 및 간격 확보 */
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.keep-row) {
        padding: 10px 12px !important; border-radius: 10px !important; margin-bottom: 10px !important; box-shadow: 0 1px 4px rgba(0,0,0,0.05);
    }
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.keep-row):hover { border-color: #0366d6 !important; background-color: #f8fbff !important; }
    
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.attendance-card-container) {
        padding: 10px 12px !important; border-radius: 10px !important; margin-bottom: 10px !important; box-shadow: 0 1px 4px rgba(0,0,0,0.05);
    }
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.attendance-card-container):hover { border-color: #0366d6 !important; background-color: #f8fbff !important; }
    
    /* ✅ 8. 출석부 토글 버튼 크기 대폭 상향 */
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.attendance-card-container) div[data-testid="stToggle"] {
        padding: 0 !important; margin: 0 !important; border: none !important; box-shadow: none !important; background-color: transparent !important; min-height: 60px !important; display: flex; align-items: center;
    }
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.attendance-card-container) div[data-testid="stToggle"] label p { font-size: 1.25rem !important; font-weight: 800 !important; color: #111 !important; }

    /* ✅ 4, 11. 이름 텍스트 버튼 1.2rem 크기 상향 및 투명화 (클릭영역 최대화) */
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.keep-row) div[data-testid="stButton"] button {
        border: none !important; background-color: transparent !important; box-shadow: none !important; text-align: left !important; justify-content: flex-start !important; padding: 5px 0px !important; color: #111 !important; font-size: 1.2rem !important; font-weight: 800 !important; height: 60px !important; 
    }
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.keep-row) div[data-testid="stButton"] button:hover { color: #0366d6 !important; }

    /* ✅ 5. 사진 사이즈 확대에 따른 모바일 레이아웃 강제 가로 유지 (85px 할당) */
    @media (max-width: 576px) {
        div[data-testid="stHorizontalBlock"]:has(.keep-row), div[data-testid="stHorizontalBlock"]:has(.attendance-card-container) { display: flex !important; flex-direction: row !important; flex-wrap: nowrap !important; align-items: center !important; }
        div[data-testid="stHorizontalBlock"]:has(.keep-row) > div[data-testid="column"]:nth-child(1), div[data-testid="stHorizontalBlock"]:has(.attendance-card-container) > div[data-testid="column"]:nth-child(1) { width: 85px !important; min-width: 85px !important; flex: 0 0 85px !important; }
        div[data-testid="stHorizontalBlock"]:has(.keep-row) > div[data-testid="column"]:nth-child(2), div[data-testid="stHorizontalBlock"]:has(.attendance-card-container) > div[data-testid="column"]:nth-child(2) { flex: 1 1 auto !important; min-width: 0 !important; }
        
        /* ✅ 10. 모바일 모달창 패딩 최소화로 화면 꽉 차게 */
        div[role="dialog"] > div { padding: 1.5rem 1rem !important; }
    }

    /* ✅ 16. 하단 '닫기' 버튼(Sticky Footer) 터치 높이 최적화 */
    div[data-testid="stVerticalBlock"]:has(.sticky-footer-marker) {
        position: sticky !important; bottom: -25px !important; background-color: white !important; z-index: 99999 !important; padding-top: 15px !important; padding-bottom: 20px !important; border-top: 1px solid #eef2f6 !important; box-shadow: 0 -4px 6px -1px rgba(0,0,0,0.05);
    }
            ...
            for idx_j, (_, r) in enumerate(group.iterrows()):
                ...
                with stu_cols[idx_j % 2]:
                    with st.container(border=True):
                        ...
                        c_img, c_btn = st.columns([1.5, 4.5])
                        with c_img:
                        ...
                        with c_btn:
                            ...
                            label_text = f"{n}{suffix}{new_friend_badge}  {bd_disp}" if bd_disp else f"{n}{suffix}{new_friend_badge}"
                            if st.button(label_text, key=f"btn_link_{r['sheet_row']}", help="상세정보 확인", use_container_width=True):
...
with tabs[5]:
    ...
    with e_tabs[0]:
        if not df_act.empty:
            ...
            st.download_button(label="📄 전체 행사일정 PDF 인쇄용 다운로드 (다운로드 후 문서 열어서 인쇄 -> PDF로 저장)", data=html_event.encode('utf-8'), file_name="슈팅스타_행사일정보고서.html", mime="text/html", use_container_width=True)
            st.divider()
