...
st.markdown("""
    ...
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.attendance-card-container) {
        padding: 8px 12px !important; border-radius: 8px !important; margin-bottom: 8px !important;
    }
    ...
    /* 🔴 출석부 토글 및 글씨크기 확대(150% 매칭) */
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.attendance-card-container) div[data-testid="stToggle"] {
        padding: 0 !important; margin: 0 !important; border: none !important; box-shadow: none !important; background-color: transparent !important; height: auto !important; display: flex; align-items: center;
    }
    ...
    /* 🔴 모바일 반응형 강제 가로 유지 및 사진 컬럼 너비 조정(68px 수용) */
...
def manage_bulletin_dialog(w_str, d_str):
    ...
    if not existing_data.empty:
        if st.button("🚨 이 주차의 주보 데이터 완전 삭제", use_container_width=True):
            ...
            st.success("🗑️ 삭제 완료!"); time.sleep(1); fetch_sheet_data.clear(); st.rerun()
...
def edit_student_dialog(target_dict):
    ...
    if not st.session_state[edit_key]:
        ...
        # [6] 상세보기에서는 암호화(마스킹) 상관없이 100% 정보 노출
...
with tabs[0]:
    ...
    # [1] 반별 명단 상단 대시보드 추가
    st.markdown(f"""
    <div style="font-size: 0.95rem; color: #444; background-color: #f8f9fa; padding: 12px 15px; border-radius: 8px; border: 1px solid #ddd; margin-bottom: 20px;">
            ...
            for idx_j, (_, r) in enumerate(group.iterrows()):
                ...
                with stu_cols[idx_j % 2]:
                    with st.container(border=True):
                        ...
                        # [2] 이름 텍스트 옆에 생일과 상세기능(버튼) 통합 배치 (150% 크기 적용 68px)
                        ...
                        with c_btn:
                            # st.button 내부에 마크다운을 적용하기 위해 편법 대신, st.button 라벨만으로 구성하고 위아래 여백을 줌
    ...
    with p_tabs[1]:
        with st.form("add_new_prayer_form"):
            ...
            if st.form_submit_button("💾 기도순서 저장", type="primary"):
                ...
                st.success("🙏 기도 일정이 성공적으로 기록되었습니다."); time.sleep(1.5); fetch_sheet_data.clear(); st.rerun()
