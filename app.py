...
# --- 1. 전역 설정 및 상수 ---
st.set_page_config(page_title="유년부 통합 관리 v36.0", page_icon="🌱", layout="wide")
...
if 'jump_to_edit' not in st.session_state: st.session_state.jump_to_edit = None
if 'student_selector' not in st.session_state: st.session_state.student_selector = 0
if 'manage_mode_idx' not in st.session_state: st.session_state.manage_mode_idx = 0
...
# [신규] 자연 정렬 알고리즘 (1-10이 1-2 뒤에 오도록)
def natural_sort_key(s):
    return [int(text) if text.isdigit() else text.lower() for text in re.split('([0-9]+)', str(s))]

# [신규] 유니코드 취소선 합성 함수 (이사/졸업자용)
def strike(text):
...
with tabs[1]:
    ...
    st.info("💡 **이름을 클릭**하시면 교적부 화면으로 연동됩니다. 반 명단 하단에서 **새친구를 즉시 등록**할 수 있습니다.")
    
    # 1. 반 이름 자연 정렬 (1-1, 1-2, 1-10 순서 보장)
    ...
    for i, c_name in enumerate(all_classes):
        ...
        # 2. 직분별 가중치 부여 (교사 0순위, 새친구 1순위, 일반 2순위, 이사/졸업 3순위)
        sort_map = {'교사': 0, '새친구': 1, '일반': 2}
        group['sort_key'] = group[status_col].map(lambda x: 3 if x in INACTIVE_STATUS else sort_map.get(x, 2))
        ...
        with cols[i % 3]:
            with st.container(border=True):
                ...
                for j, (_, r) in enumerate(group.iterrows()):
                    ...
                    with btn_cols[j % 2]:
                        if st.button(label, key=f"btn_link_{r['sheet_row']}", help="클릭 시 정보 수정", use_container_width=True):
                            st.session_state.jump_to_edit = r['sheet_row']
                            st.session_state.manage_mode_idx = 1
                            st.toast(f"✅ [{n}] 선택됨! 상단의 [📋 교적부] 탭을 확인하세요.", icon="🚀")
