...
st.markdown("""
    ...
    /* ✅ 1. 탭(Tabs) 글씨 3배 확대 & 모바일 가로 스크롤 적용 */
    ...
    div[data-baseweb="tab"] { 
        flex: 0 0 auto !important; justify-content: center; padding: 15px 25px !important; margin: 0 5px 0 0 !important; 
        background-color: #f8f9fa; border-radius: 10px; border: 1px solid #eee; min-height: 80px !important;
    }
    div[data-baseweb="tab"][aria-selected="true"] { background-color: #0366d6 !important; color: white !important; border: 1px solid #0366d6; }
    
    /* 탭 메뉴 글씨 3배 확대 (약 2.5rem) */
    div[data-baseweb="tab"] p { font-size: 2.5rem !important; font-weight: 800 !important; white-space: nowrap; margin: 0; }
    
    /* ✅ 모든 화면 위치에서 우측 하단에 고정되는 '맨 위로' 버튼 */
    .fab-button { position: fixed; bottom: 25px; right: 25px; left: auto; background-color: rgba(3, 102, 214, 0.9); color: white !important; padding: 15px 20px; border-radius: 30px; text-decoration: none; font-weight: 800; font-size: 1.1rem; box-shadow: 0 4px 10px rgba(0,0,0,0.3); z-index: 999999; }
    
    /* ✅ 2. 반별명단 학생 카드 및 투명 오버레이 버튼 기법 */
    ...
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.attendance-card-container) div[data-testid="stToggle"] {
        ...
        background-color: transparent !important; min-height: 80px !important; display: flex; align-items: center;
    }
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.attendance-card-container) div[data-testid="stToggle"] label p {
        font-size: 2.0rem !important; /* 이름 폰트 2배 */
        font-weight: 800 !important;
    }
    /* 스위치 2.5배 확대 */
    div[data-testid="stVerticalBlockBorderWrapper"]:has(.attendance-card-container) div[data-testid="stToggle"] div[data-baseweb="checkbox"] > div:first-child {
        transform: scale(2.5) !important;
        transform-origin: left center !important;
        margin-right: 40px !important; /* 커진 스위치 옆 여백 */
    }
    ...
    @media (max-width: 576px) {
        ...
        div[data-testid="stHorizontalBlock"]:has(.keep-row) > div[data-testid="column"]:nth-child(1),
            ...
            for idx_j, (_, r) in enumerate(group.iterrows()):
                ...
                with stu_cols[idx_j % 2]:
                    with st.container(border=True):
                        ...
                        with c_info:
                            # 폰트 사이즈 조정 (이름 2.0rem, 생일 1.0rem 유지)
                            info_html = f'''
                            <div style="height:60px; display:flex; align-items:center;">
                                <span style="font-size:2.0rem; font-weight:800; color:#111; margin-right:8px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{n}{suffix}{new_friend_badge}</span>
