...
# 📱 모바일 UI 최적화 적용 CSS (버튼/입력창 확대, 여백, 플로팅 버튼)
st.markdown("""
    <style>
    ...
    div[data-testid="stToggle"] { border: 2px solid #eef2f6; padding: 12px 15px; border-radius: 12px; background-color: #ffffff; box-shadow: 0 4px 6px rgba(0,0,0,0.05); margin-bottom: 12px; }
...
def edit_student_dialog(target_dict):
    ...
    if not st.session_state[edit_key]:
        ...
        if clean_p_url and str(clean_p_url).startswith('http'): st.markdown(f'<img src="{clean_p_url}" style="width:100%; max-width:300px; border-radius:15px; margin:auto; display:block; margin-bottom:20px;">', unsafe_allow_html=True)
        ...
        with st.form("modal_edit_form"):
            clean_p_url = safe_str(target_dict.get('사진', '')).replace("&vid=1", "").replace("?vid=1", "")
            if clean_p_url and str(clean_p_url).startswith('http'): st.markdown(f'<img src="{clean_p_url}" style="width:100%; max-width:200px; border-radius:15px; margin-bottom:10px;">', unsafe_allow_html=True)
...
with tabs[0]:
    ...
    selected_class = st.selectbox("👇 조회할 반을 선택하세요", ["반을 선택하세요", "전체 명단 한눈에 보기(느려질 수 있음)"] + all_classes)
    
    if selected_class != "반을 선택하세요":
        class_list_to_show = all_classes if selected_class == "전체 명단 한눈에 보기(느려질 수 있음)" else [selected_class]
        
        for c_name in class_list_to_show:
            ...
            st.markdown(f"<div class='class-header'>🏷️ {c_name} ({active_count}명)</div>", unsafe_allow_html=True)
            
            # 1반씩 보여줄 때는 사진과 버튼을 나란히
            ...
            with st.expander(f"➕ 새친구 빠른 추가"):
                with st.form(f"qa_{c_name}"):
                    new_n = st.text_input("이름", placeholder="이름을 입력하세요")
                    if st.form_submit_button("등록", type="primary", use_container_width=True) and new_n:
                        new_row = [""] * len(headers)
...
with tabs[4]:
    ...
    if not news.empty: 
        if st.session_state.get('privacy_mode', True):
            for c_priv in ['연락처', '주소']:
                if c_priv in news.columns: news[c_priv] = "🔒 [보호됨]"
        st.dataframe(news[['이름', class_col, '연락처', '비고']], use_container_width=True, hide_index=True)
    else: st.info("새친구가 없습니다.")

# ==========================================
# [탭 5] 행사 (서브 탭 UI & HTML PDF 보고서)
# ==========================================
with tabs[5]:
    st.markdown('<a href="#top-anchor" class="fab-button">⬆ 맨 위로</a>', unsafe_allow_html=True)
    st.subheader("⚙️ 행사 기록 관리")
    
    t_ev_view, t_ev_add, t_ev_edit, t_ev_del = st.tabs(["📂 내용 보기/다운로드", "➕ 행사 등록", "📝 내용 수정", "🚨 삭제"])
...
with tabs[6]:
    ...
    with st.container(border=True):
        ...
        sel_class = st.selectbox("반 필터 (모바일 속도 향상을 위해 반 선택 권장)", ["반을 선택하세요", "전체 명단 불러오기"] + all_classes_list)
    ...
    if sel_class != "반을 선택하세요":
        ...
        with st.form("att_toggle_form"):
            ...
            for c_name in sorted(grouped.groups.keys(), key=class_sort_key):
                ...
                # 프로필 사진과 버튼을 깔끔하게 보여주기 위해 컨테이너 반복
                for idx, row in grouped.get_group(c_name).iterrows():
                    ...
                    clean_p_url = str(row.get('사진', '')).replace("&vid=1", "").replace("?vid=1", "")
                    
                    with st.container(border=True):
                        c_img, c_tgl = st.columns([1, 4])
                        with c_img:
                            if clean_p_url and clean_p_url.startswith('http'):
                                st.markdown(f'<img src="{clean_p_url}" style="width:45px; height:45px; border-radius:50%; object-fit:cover; border:2px solid #eee; margin-top:5px;">', unsafe_allow_html=True)
                            else:
                                st.markdown("<div style='font-size:2rem; text-align:center;'>👤</div>", unsafe_allow_html=True)
                        with c_tgl:
                            new_att[row['sheet_row']] = st.toggle(f"{prefix}{row['이름']}", value=is_on, key=f"tgl_{row['sheet_row']}_{sel_w}")
